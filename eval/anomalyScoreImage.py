...
def main():
    ...
    parser.add_argument('--method', default="Max_logits")
    ...
    for path in glob.glob(os.path.expanduser(str(args.input))): #args.input[0] -> args.input 
        ...
        if(args.method == "Max_logits"):
            ...
            anomaly_result = torch.div(torch.sum(-probs * torch.log(probs), dim=1), torch.log(torch.tensor(probs.shape[1])))
    ...
    n_ood = np.count_nonzero(ood_mask)
    n_ind = np.count_nonzero(ind_mask)
    n_tot = n_ood+n_ind

    print("OOD/IND pixel ratio percentage:" + str(n_ood/n_tot*100) + "%")
